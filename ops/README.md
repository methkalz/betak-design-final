...
## أيّ خادمٍ عندك؟ (يحدّد أيّ ملفّ تستعمل)

```bash
curl -s https://betakd.com/مسار-غير-موجود | head -3
```

- صفحة خطأ **Apache** («The requested URL was not found on this server») ←
  خادمك Apache، أو nginx واجهةً وApache خلفه (بنية cPanel/WHM). استعمل
  **`htaccess-spa.conf`**: ارفعه باسم `.htaccess` بجانب `index.html`.
  ملفّ nginx أدناه **لن يُستعمل**.
- صفحة خطأ **nginx** ← استعمل `nginx-betakd.conf` كما في القسم التالي.

الفارق ليس تجميليًّا: بلا الملفّ الصحيح يعطي `/project/123` خطأ 404 عند إعادة
تحميل الصفحة - يعمل التنقّل داخل التطبيق ثم ينكسر أوّل تحديثٍ أو رابطٍ محفوظ.

## أوّل مرّة على الخادم (nginx وحده)
