-- ════════════════════════════════════════════════════════════════════
-- أوامر الإنتاج تكتب إلى الخادم: الإسناد وتقدّم المراحل
--
-- assign_tailor (أدمن): أمر مفتوح واحد لكل مشروع تحت قفله، والمشروع
-- «مع الخياط»، والخياط يُخطر لحظته. advance_stage (الأدمن أو خياط
-- الأمر): خطوة واحدة في الاتجاهين، و«جاهز» لا يصح وفي المشروع شباك بلا
-- تأكيد إنهاء - ثم «جاهز للتركيب» بإشعارَي المركّب والإدارة (M18).
-- بحث الإعادة قبل حُرّاس الحالة، وسجل المراحل {stage, at} كما يقرؤه
-- التطبيق حرفًا.
-- ════════════════════════════════════════════════════════════════════

-- ── منحة الإدراج: أول RPC ينشئ أوامر إنتاج ─────────────────────────────
grant insert on core.tailor_assignments to baytak_rpc_owner;
...
alter function api.assign_tailor(uuid, uuid, uuid, text, timestamptz) owner to baytak_rpc_owner;
alter function api.advance_stage(uuid, text, uuid) owner to baytak_rpc_owner;
