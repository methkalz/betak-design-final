-- ============================================================================
-- اللقطة الذرية للتسعير — إغلاق اعتراض المراجعة: «pricing_context ليست ذرية
-- مع الحساب».
--
-- العلة المؤكدة: النسخة السابقة كانت تقرأ الإعدادات في عبارة، وتجمع القواعد
-- في عبارة ثانية، ثم يعيد المحرك قراءة الجداول الحية في عبارة ثالثة — وكل
-- عبارة في READ COMMITTED ترى لقطة أحدث، فيمكن نظريًا أن يوثق السياق قاعدةً
-- وتُحسب البنود بقاعدة أحدث التزمت بين العبارتين.
--
-- الإصلاح البنيوي:
--   1) السياق كله يُبنى في «عبارة واحدة» (إعدادات + قواعد = لقطة واحدة).
--   2) المحرك private.price_project_windows يستقبل السياق ويقرأ منه حصرًا
--      كل مدخلات السياسة (القواعد وتكاليف المكونات وبطانة الافتراض) — لا
--      قراءة حية لأي جدول سياسة أثناء بناء البنود. الشبابيك والغرف وتكلفة
--      قماش الـvariant تبقى حية (محتوى لا سياسة؛ البنود تلقطها لحظتها).
--   النتيجة: البنود مطابقة للسياق «بالبناء» مهما تزامنت التعديلات.
-- ============================================================================

drop function private.price_project_windows(uuid, uuid);
...
revoke all on function private.price_project_windows(uuid, uuid, jsonb) from public;
grant execute on function private.price_project_windows(uuid, uuid, jsonb) to baytak_rpc_owner;


-- ────────────────────────────────────────────────────────────────────────────
-- api.create_quotation_version — السياق يُبنى في عبارة واحدة ويُمرر للمحرك
-- ────────────────────────────────────────────────────────────────────────────
