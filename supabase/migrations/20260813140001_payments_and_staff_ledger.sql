-- ════════════════════════════════════════════════════════════════════
-- المال يكتب إلى الخادم: الدفعات والشيكات ودفتر الطاقم
--
-- record_payment يفرض عقد DECISIONS §11 حرفًا: لا دفعة قبل عرضٍ معتمد
-- (BD409)، والشيكل صحيح دائمًا. record_check_series يدخل الرزمة معاملةً
-- واحدة بمرجع CHK i/N وموعد صرف لكل شيك. reverse_payment قيدُ عكسٍ
-- سالب مرتبط بأصله - القيد لا يُعكس مرتين ولا يُعكس عكسٌ. دفتر الطاقم
-- جدول جديد insert-only بمحفّز الجمود، ودفعته إشعارٌ لصاحبها (M8).
-- عمودا الشيكات (due_at وصورة الرزمة) يصلان جدول الدفعات وواجهته.
-- أسئلة المالك الثلاثة المعلقة (سقف الدفعات، أثر نسخة لاحقة، عقد العكس
-- التفصيلي) تبقى معلقة - هذه مرآة التطبيق كما هو، لا قرارات جديدة.
-- ════════════════════════════════════════════════════════════════════

-- ── أعمدة الشيكات على الدفعات ───────────────────────────────────────────
alter table core.payments
  add column if not exists due_at timestamp with time zone,
  add column if not exists photo_uri text not null default '';

-- ── دفتر الطاقم: قيود الدفع للموظفين - insert-only كسائر الدفاتر ────────
create table if not exists core.staff_ledger (
...
create trigger staff_ledger_immutable before delete or update on core.staff_ledger
  for each row execute function private.block_mutation();

alter table core.staff_ledger enable row level security;
-- FORCE يشمل المالك أيضًا (عرف البيت منذ 2.10) - وbypassrls لدور الـRPC وحده
alter table core.staff_ledger force row level security;
-- الأدمن يرى الدفتر كله، والموظف قيوده هو - كتابةً لا سبيل غير الـRPC
...
grant select on core.staff_ledger to baytak_rpc_owner, authenticated;
grant insert on core.staff_ledger to baytak_rpc_owner;
...
grant select on api.staff_ledger to authenticated;
...
alter function api.record_payment(uuid, bigint, text, text, uuid, text, text, timestamptz) owner to baytak_rpc_owner;
alter function api.record_check_series(uuid, jsonb, uuid, text) owner to baytak_rpc_owner;
alter function api.reverse_payment(uuid, text, uuid) owner to baytak_rpc_owner;
alter function api.record_staff_payout(uuid, bigint, uuid, text) owner to baytak_rpc_owner;

revoke create on schema api from baytak_rpc_owner;

revoke all on function
  api.record_payment(uuid, bigint, text, text, uuid, text, text, timestamptz),
  api.record_check_series(uuid, jsonb, uuid, text),
  api.reverse_payment(uuid, text, uuid),
  api.record_staff_payout(uuid, bigint, uuid, text)
from public, anon;
