-- ════════════════════════════════════════════════════════════════════
-- القياسات تكتب إلى الخادم: الغرف والشبابيك
--
-- add/delete_room و save/delete_window بأدوار القياس نفسها (أدمن وبائع
-- وميداني)، بتحقّقات التطبيق حرفًا (لا شباك بلا قماشه، البطانة بلونها،
-- المضاعف 1.5-4، السقف 500 سم)، وأول قياس يحرّك المشروع «تم القياس».
-- الحذف للتصحيح المبكر حصرًا: شباكٌ مسجَّل الإنجاز أو غرفته لا يُحذفان -
-- فحذفهما يفكّ قيود الاستهلاك عن شباكها ويضيع أثر «مُنجز».
-- ════════════════════════════════════════════════════════════════════

-- ── منح المالك: أول RPCs تكتب الغرف والشبابيك ───────────────────────────
grant insert, delete on core.rooms to baytak_rpc_owner;
grant insert, update, delete on core.windows to baytak_rpc_owner;
...
alter function api.add_room(uuid, text, uuid, text) owner to baytak_rpc_owner;
alter function api.delete_room(uuid, uuid) owner to baytak_rpc_owner;
alter function api.save_window(uuid, uuid, numeric, numeric, uuid, uuid, uuid, text, boolean, uuid, text, numeric, integer, text) owner to baytak_rpc_owner;
alter function api.delete_window(uuid, uuid) owner to baytak_rpc_owner;

revoke create on schema api from baytak_rpc_owner;

revoke all on function
  api.add_room(uuid, text, uuid, text),
  api.delete_room(uuid, uuid),
  api.save_window(uuid, uuid, numeric, numeric, uuid, uuid, uuid, text, boolean, uuid, text, numeric, integer, text),
  api.delete_window(uuid, uuid)
from public, anon;
