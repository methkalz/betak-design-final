-- ============================================================================
-- بيتك ديزاين — 0023 — consume_fabric يحترم released_m
--
-- بعد إضافة released_m في 0022، صار حساب المتبقي في الاستهلاك ناقصًا:
--     كان:   quantity_m − consumed_m
--     الصواب: quantity_m − consumed_m − released_m
-- بدونه يعتبر النظام كميةً حُرِّرت وكأنها ما زالت محجوزة، فيستهلك منها
-- «داخل الحجز» بدل أن يعدّها زيادة تستوجب سببًا وإشعارًا.
-- ============================================================================
...
begin
  ...
  select p.status_code, p.lock_version, p.tailor_id
    into v_lock_ver, v_lock_ver, v_tailor
  ...
  -- حركتان منفصلتان: الجزء المحجوز، ثم الزيادة التي لا تمس reserved
