-- ============================================================================
-- بيتك ديزاين — 0018 — private.current_uid() وإصلاح reserve_fabric
--
-- المشكلة: كل دالة RPC مملوكة لـbaytak_rpc_owner، و security definer ينفّذها
-- بصلاحيات مالكها. المالك لا يملك USAGE على سكيما auth، فأول سطر ينادي
-- auth.uid() كان يرمي 42501 — أي أن الدالة كانت معطّلة كليًا.
--
-- ولا يمكن منحه USAGE: السكيما مملوكة لـsupabase_admin، و postgres يحملها
-- بلا GRANT OPTION. محاولة المنح تصدر WARNING لا ERROR فتمر صامتة (انظر 0017).
--
-- الحل: إزالة الاعتماد. auth.uid() ليست إلا قراءة GUC — هذا تعريفها الفعلي
-- على هذا البناء:
--     coalesce(nullif(current_setting('request.jwt.claim.sub', true), ''),
--              nullif(current_setting('request.jwt.claims', true), '')::jsonb ->> 'sub'
--     )::uuid
-- و current_setting في pg_catalog، متاحة للجميع بلا أي اعتماد على سكيما.
--
-- الفائدة الجانبية: كل RPC قادم يستعمل نفس الدالة، فلا يتكرر العطل.
-- ============================================================================
...
comment on function private.current_uid() is
  'مكافئ auth.uid() بلا اعتماد على سكيما auth. تستعمله كل دوال RPC المملوكة '
  'لـbaytak_rpc_owner، إذ لا يملك USAGE على auth ولا يمكن منحه إياها.';

grant execute on function private.current_uid() to authenticated, baytak_rpc_owner;

-- ════════════════════════════════════════════════════════════════════════════
-- إعادة إنشاء reserve_fabric — التغيير الوحيد: private.current_uid() بدل auth.uid()
-- ════════════════════════════════════════════════════════════════════════════
...
revoke all on function api.reserve_fabric(uuid, uuid, numeric, uuid, integer)
  from public, anon;
grant execute on function api.reserve_fabric(uuid, uuid, numeric, uuid, integer)
  to authenticated;
