...
begin
  ...
  -- قالب الوثيقة نصٌّ من قائمةٍ مغلقة: حارسٌ خاصٌّ به لأن سلاسل النسب
  -- (coalesce(x,0)) لا معنى لها هنا - النصّ لا صفرَ له
  if p_quote_template is not null and p_quote_template not in
     ('onyx','swiss','panel','linen','ink','azure','atelier','ledger',
      'seal','weave','curve','folds') then
    raise exception 'قالب وثيقة غير معروف.' using errcode = 'BD400';
  end if;
  ...
  v_payload := jsonb_build_object(
    ...
    'oversize_pct', p_oversize_surcharge_percent, 'template', p_quote_template);
  ...
  update core.business_settings set
    ...
    quote_template                         = coalesce(p_quote_template, quote_template),
    updated_at                             = now()
