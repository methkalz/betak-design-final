...
--
-- كانت «إضافة موظف» و«تعطيله» ترفضان بصدق لأن الحساب يمسّ auth.users.
-- النمط المجرَّب على الإنتاج (سكربت البذرة) يُغلَّف هنا بحارس الأدمن:
-- أعمدة رموز GoTrue تُدرَج '' لا NULL، وصفُّ هويةٍ لكل حساب.
--
-- والصلاحيات على قاعدة السقف: الدور يرسمه والأدمن يُطفئ ما تحته -
-- {capability: false} إيقافاتٌ فقط، والمنح فوق الدور طريقه رفعُ الدور
-- فتسري القاعدة على الخادم كله لا على شاشةٍ وحدها. والتعطيل يقفل الباب:
-- حظرٌ في auth ومحوُ الجلسات، لا إخفاءُ صفٍّ في شاشة.
--
-- وتبقى الخمس بملكية postgres لا baytak_rpc_owner - عمدًا: قوّة postgres
-- على auth هي جوهر عملها، ومنحُها للمالك المعتاد لا-أثرَ صامت (01007).
-- ════════════════════════════════════════════════════════════════════
