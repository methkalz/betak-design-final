-- ════════════════════════════════════════════════════════════════════
-- لوحة الأدمن تكتب التسعيرة إلى الخادم
--
-- update_business_settings يرقّع ما تغيّر فقط (16 حقلًا اختياريًا)
-- بمرايا التطبيق: لا سالب، النسب في مداها، وسقف الموظف لا يتجاوز سقف
-- الأدمن - على القيم النهائية بعد الدمج، وبعد بحث الإعادة كي يصمد عقد
-- idempotency أمام تغيّر الحالة اللاحق. غياب صف الإعدادات BD404 لا نجاح
-- صامت. update_pricing_rule بالمفتاح الثابت (الشريحة والفئة) لا بمعرّف
-- الصف. التعديل يسري على العروض القادمة وحدها: المقفول يسعّر من لقطته
-- إلى الأبد (§10).
-- ════════════════════════════════════════════════════════════════════

-- ── منح المالك: أول RPCs تكتب الإعدادات والقواعد ────────────────────────
grant update on core.business_settings to baytak_rpc_owner;
grant update on core.pricing_rules to baytak_rpc_owner;
...
alter function api.update_business_settings(uuid, bigint, bigint, bigint, bigint, numeric, numeric, numeric, integer, numeric, bigint, bigint, bigint, bigint, bigint, bigint, bigint) owner to baytak_rpc_owner;
alter function api.update_pricing_rule(text, text, bigint, bigint, uuid) owner to baytak_rpc_owner;

revoke create on schema api from baytak_rpc_owner;

revoke all on function
  api.update_business_settings(uuid, bigint, bigint, bigint, bigint, numeric, numeric, numeric, integer, numeric, bigint, bigint, bigint, bigint, bigint, bigint, bigint),
  api.update_pricing_rule(text, text, bigint, bigint, uuid)
from public, anon;
