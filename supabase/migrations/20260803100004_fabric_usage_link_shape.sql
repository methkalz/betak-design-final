-- ============================================================================
-- بيتك ديزاين — 0032 — سدّ تجاوز NULL في رابط الاستخدام + سبب الإرجاع
--
-- الثغرة (مراجعة الطاقم، ثم تأكيد تجريبي داخل ROLLBACK قبل هذا الإصلاح):
-- الـFK الخماسي يعمل بـ MATCH SIMPLE، وأي NULL في أعمدته يعفي الصف من
-- المطابقة كليًا. فالإدراج التالي كان **يُقبل**:
--     type='return', fabric_usage_id=<سجل صحيح>, roll_id=<رول آخر!>,
--     project_id=NULL, reservation_id=NULL, reason=''
-- أي أن ادعاء «المحرّك يفرض التطابق في كل المسارات» كان خاطئًا بصيغته تلك.
--
-- MATCH FULL ليس الحل: كان سيكسر كل الحركات غير المرتبطة بالاستخدام التي
-- تحمل project أو reservation مع fabric_usage_id فارغ. الصحيح: قيد شكل
-- يفرض السياق الكامل على return حصرًا، فيصير الـFK فعالًا فعلًا.
-- ============================================================================

-- قيد الشكل يبتلع return_requires_usage القديم ويتجاوزه
alter table core.stock_movements drop constraint return_requires_usage;

alter table core.stock_movements add constraint fabric_usage_link_shape
  check (
    (
      type = 'return'
      and fabric_usage_id is not null
    ...
    (
      type <> 'return'
      and fabric_usage_id is null
    )
  );

comment on constraint fabric_usage_link_shape on core.stock_movements is
  'كل return يحمل السياق الكامل (usage + project + reservation) فلا يستطيع '
  'NULL إعفاءه من الـFK الخماسي، وأي حركة غير return لا تحمل fabric_usage_id. '
  'مع القيد الخماسي يصير تطابق الرول والحجز والمشروع مفروضًا فعلًا لا ادعاءً.';

-- الإرجاع يحرك المخزون صعودًا — سببه إلزامي كالتلف والتسويات (قرار: reason_code
-- إلزامي و notes اختيارية، ليكون التجميع في التقارير ممكنًا)
...
-- ════════════════════════════════════════════════════════════════════════════
-- توحيد مدخلات البصمة (بصمة v3): الكمية مقرَّبة لثلاث منازل، السبب والملاحظات
-- بعد btrim، و NULL و '' تمثيل واحد — والقيمة المطبَّعة نفسها هي المخزنة.
-- بدونه: مسافة زائدة = «عملية مختلفة»، أو تُخزن قيمة غير التي حُسبت منها الهوية.
-- يُطبَّق بأثر رجعي على الدالتين القائمتين كي لا يعيش جيلان من الانضباط.
-- ════════════════════════════════════════════════════════════════════════════
