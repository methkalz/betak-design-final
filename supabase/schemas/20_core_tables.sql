...
CREATE TABLE core.business_settings (
    ...
    CONSTRAINT business_settings_oversize_surcharge_percent_check CHECK (((oversize_surcharge_percent >= (0)::numeric) AND (oversize_surcharge_percent <= (100)::numeric))),
    CONSTRAINT business_settings_quotation_validity_days_check CHECK ((quotation_validity_days > 0)),
    CONSTRAINT business_settings_quote_template_check CHECK ((quote_template = ANY (ARRAY['onyx'::text, 'swiss'::text, 'panel'::text, 'linen'::text, 'ink'::text, 'azure'::text, 'atelier'::text, 'ledger'::text, 'seal'::text, 'weave'::text, 'curve'::text, 'folds'::text]))),
...
CREATE TABLE core.quotation_items (
    ...
    list_price_agorot bigint DEFAULT 0 NOT NULL,
    ...
    CONSTRAINT quotation_items_list_price_agorot_check CHECK ((list_price_agorot >= 0)),
...
CREATE TABLE core.quotation_versions (
    ...
    note text DEFAULT ''::text NOT NULL,
    markup_spec jsonb DEFAULT '{}'::jsonb NOT NULL,
    created_by uuid NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
